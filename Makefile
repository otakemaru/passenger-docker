...
build_ruby23_nodejs:
	...
	docker build -t $(NAME)-ruby23-nodejs:$(VERSION) --rm ruby23_nodejs_image
...
build_ruby24_nodejs:
	...
	docker build -t $(NAME)-ruby24-nodejs:$(VERSION) --rm ruby24_nodejs_image
...
tag_latest:
	...
	docker tag $(NAME)-ruby23-nodejs:$(VERSION) $(NAME)-ruby23-nodejs:latest
	docker tag $(NAME)-ruby24-nodejs:$(VERSION) $(NAME)-ruby24-nodejs:latest
...
release: tag_latest
	...
	@if ! docker images $(NAME)-ruby23-nodejs | awk '{ print $$2 }' | grep -q -F $(VERSION); then echo "$(NAME)-ruby23-nodejs version $(VERSION) is not yet built. Please run 'make build'"; false; fi
	@if ! docker images $(NAME)-ruby24-nodejs | awk '{ print $$2 }' | grep -q -F $(VERSION); then echo "$(NAME)-ruby24-nodejs version $(VERSION) is not yet built. Please run 'make build'"; false; fi
	...
	docker push $(NAME)-ruby23-nodejs
	docker push $(NAME)-ruby24-nodejs
